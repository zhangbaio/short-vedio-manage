(() => {
  ...
  function renderRows(items) {
    ...
    if (!items.length) {
      tbody.innerHTML = `<tr><td colspan="${EMPTY_COLSPAN}" class="text-center text-muted py-4">暂无小程序上传记录</td></tr>`;
      mobileList.innerHTML = '<div class="mobile-empty text-muted">暂无小程序上传记录</div>';
      return;
    }
  ...
  async function exportCurrentFilter() {
    ...
    downloadCsv("小程序短剧上传记录.csv", rows);
  }
