(() => {
  ...
  function renderRows(items) {
    ...
    if (!items.length) {
      tbody.innerHTML = `<tr><td colspan="${EMPTY_COLSPAN}" class="text-center text-muted py-4">暂无视频号上传记录</td></tr>`;
      mobileList.innerHTML = '<div class="mobile-empty text-muted">暂无视频号上传记录</div>';
      return;
    }
    items.forEach((item, index) => {
      ...
      card.innerHTML = `
        ...
        <div class="mobile-record-grid">
          ...
          <div><span>集数</span><strong>${escapeHtml(item.episodes || "-")}</strong></div>
  ...
  async function exportCurrentFilter() {
    ...
    downloadCsv("视频号短剧上传记录.csv", rows);
  }

  function downloadCsv(filename, rows) {
    const header = ["记录时间", "用户", "原剧名", "新剧名", "集数", "上传进度", "上传状态", "上传者", "公司"];
